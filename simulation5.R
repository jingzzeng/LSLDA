## ---------------------------------------- ##
# This file implements the LSLDA algorithm on simulated data generated from Model (M5) (Section 5).
## ---------------------------------------- ##
rm(list = ls())
...
for(i in 1:5){
  theta[(2*(i-1)+1):(2*i),i] <- 2
}
theta[,6] <- theta[,1:5] %*% rep(0.5, 5)
U <- Sigma %*% theta
Mu <- cbind(rep(0,p), U)
beta <- svd(theta)$u[,1:r]
