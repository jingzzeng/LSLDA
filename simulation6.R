## ---------------------------------------- ##
# This file implements the LSLDA algorithm on simulated data generated from Model (M6) (Section 5).
## ---------------------------------------- ##
rm(list = ls())
...
foo <- function(i){
  ...
  # ---------------- lslda --------------- #
  fit_lslda <- lslda_val(x_train, y_train, x_val, y_val, n1=20, metric = "acc")
  ...
  if(is.null(beta_lslda)){
    ...
    err_lslda <- 1 - score_func(pred_lslda$class, y_test, metric = "acc")
